...
class AcademicStanding(BaseModel):
    ...
    @field_validator("completed_courses", "waived_or_transferred", "in_progress_courses")
    @classmethod
    def _normalize_course_codes(cls, codes: list[str]) -> list[str]:
        """Catalog CourseIDs are stored compact/uppercase (e.g. 'CS505'), but
        free text and transcripts write them as 'CS 505'. Normalize here so
        every consumer (reasoning.py, aiInfo.py) can compare directly against
        catalog.CourseID without each caller re-implementing this."""
        return [re.sub(r"\s+", "", c).strip().upper() for c in codes if c and c.strip()]
